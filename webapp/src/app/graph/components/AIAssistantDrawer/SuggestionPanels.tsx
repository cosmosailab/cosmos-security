...
export function SuggestionPanels({
  isConnected,
  setInputValue,
}: SuggestionPanelsProps) {
  const [openTemplateGroup, setOpenTemplateGroup] = useState<string | null>(
    null,
  );
  const [openInfoSubGroup, setOpenInfoSubGroup] = useState<string | null>(null);
  const [openExploitSubGroup, setOpenExploitSubGroup] = useState<string | null>(
    null,
  );
    ...
    <div className={styles.templateGroups}>
      ...
      <div className={styles.templateGroup}>
        <button
          ...
          onClick={() =>
            setOpenTemplateGroup((prev) =>
              prev === 'informational' ? null : 'informational',
            )
          }
          style={
            { '--tg-color': 'var(--text-tertiary)' } as React.CSSProperties
          }
        >
          <Shield size={14} />
          <span>정보 수집</span>
            ...
            {INFORMATIONAL_GROUPS.map((group) => (
              <React.Fragment key={group.id}>
                ...
                >
                  ...
                  <ChevronDown
                    size={12}
                    className={styles.templateSubGroupChevron}
                  />
                </button>
                  ...
                  <div className={styles.templateSubGroupItems}>
                    {group.items.map((section, i) => (
                      <React.Fragment key={i}>
                        {section.osLabel && (
                          <span className={styles.templateOsLabel}>
                            {section.osLabel}
                          </span>
                        )}
      ...
      <div className={styles.templateGroup}>
        <button
          ...
          onClick={() =>
            setOpenTemplateGroup((prev) =>
              prev === 'exploitation' ? null : 'exploitation',
            )
          }
          style={
            { '--tg-color': 'var(--status-warning)' } as React.CSSProperties
          }
        >
          <Target size={14} />
          <span>익스플로잇</span>
      ...
      <div className={styles.templateGroup}>
        <button
          ...
          onClick={() =>
            setOpenTemplateGroup((prev) =>
              prev === 'post_exploitation' ? null : 'post_exploitation',
            )
          }
