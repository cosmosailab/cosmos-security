...
function headerIcon(status: FireteamItem['status']) {
  switch (status) {
    case 'running':
      return (
        <Loader2 size={14} className={`${styles.icon} ${styles.spinner}`} />
      );
    case 'completed':
      return (
        <CheckCircle2
    ...
    case 'timeout':
      return (
        <Hourglass size={14} className={`${styles.icon} ${styles.iconError}`} />
      );
    case 'failed':
      return (
        <XCircle size={14} className={`${styles.icon} ${styles.iconError}`} />
      );
    case 'cancelled':
...
export function FireteamCard({
  ...
  if (counts.success) countStrs.push(`${counts.success}개 성공`);
  if (counts.timeout) countStrs.push(`${counts.timeout}개 시간 초과`);
  if (counts.error) countStrs.push(`${counts.error}개 오류`);
  if (counts.cancelled) countStrs.push(`${counts.cancelled}개 취소`);
  if (counts.needs_confirmation)
    countStrs.push(`${counts.needs_confirmation}개 승인 필요`);
    ...
    <div className={`${styles.card} ${styles[`status_${item.status}`] || ''}`}>
      ...
      >
        ...
        <span className={styles.title}>
          공격조 · 단계 {item.iteration} · {item.members.length}명의 전문가
        </span>
        {headerIcon(item.status)}
        <span className={styles.statusLabel}>{item.status}</span>
        <span className={styles.meta}>
          {countStrs.join(' · ')}
          {item.wall_clock_seconds !== undefined &&
            ` · ${item.wall_clock_seconds.toFixed(1)}s`}
        </span>
      </button>
