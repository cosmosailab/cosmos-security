    ...
    switch (status) {
      ...
      case 'in_progress':
        return (
          <Loader2
            size={14}
            className={`${styles.iconInProgress} ${styles.spinner}`}
          />
        );
      case 'pending':
      default:
  ...
  return (
    <div className={styles.todoWidget}>
      ...
      <div className={styles.todoHeader}>
        <span className={styles.todoLabel}>할 일</span>
        <button
          className={styles.expandButton}
          onClick={() => setIsExpanded(!isExpanded)}
          title={isExpanded ? '목록 접기' : '목록 펼치기'}
        >
      ...
      {!isExpanded && currentTask && (
        <div
          className={`${styles.todoItem} ${getStatusClass(currentTask.status)}`}
        >
        ...
        <div className={styles.todoListExpanded}>
          {items.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className={`${styles.todoItem} ${getStatusClass(item.status)}`}
              >
                <div className={styles.todoIcon}>
