...
export function useConversationRestoration(deps: ConversationRestorationDeps) {
  ...
  const handleSelectConversation = useCallback(
    async (conv: Conversation) => {
      ...
      const standaloneStartsByName = new Map<
        string,
        { id: string; createdAt: string }[]
      >();
      const standaloneCompletesByName = new Map<
        string,
        { id: string; createdAt: string }[]
      >();
      for (const msg of full.messages) {
        ...
        if (
          msg.type === 'tool_start' &&
          !d?.wave_id &&
          !duplicateStartIds.has(msg.id)
        ) {
          const name = d?.tool_name || '';
          if (!standaloneStartsByName.has(name))
            standaloneStartsByName.set(name, []);
          standaloneStartsByName
            .get(name)!
            .push({ id: msg.id, createdAt: msg.createdAt });
        }
        if (
          msg.type === 'tool_complete' &&
          !d?.wave_id &&
          !duplicateCompleteIds.has(msg.id)
        ) {
          const name = d?.tool_name || '';
          if (!standaloneCompletesByName.has(name))
            standaloneCompletesByName.set(name, []);
          standaloneCompletesByName
            .get(name)!
            .push({ id: msg.id, createdAt: msg.createdAt });
        }
      }
      ...
      for (const [name, completes] of standaloneCompletesByName) {
        ...
        for (let i = 0; i < completes.length && i < starts.length; i++) {
          ...
          completeToStartTime.set(
            completes[i].id,
            new Date(starts[i].createdAt),
          );
        }
      }
      ...
      const restored: ChatItem[] = full.messages
        .map(
          (msg: {
            id: string;
            type: string;
            ...
            if (
              ...
              if (lastToolConfirmationRequest)
            ...
            if (
              ...
              const restoredTier =
                data.response_tier ||
                (data.task_complete ? 'full_report' : undefined);
              return {
                type: 'message',
                id: msg.id,
                ...
                isReport:
                  restoredTier === 'full_report' ||
                  (!data.response_tier &&
                    (data.isReport || data.task_complete || false)),
              ...
              const duration = startTime
                ? completeTime.getTime() - startTime.getTime()
                : undefined;
              return {
                type: 'tool_execution',
                id: msg.id,
              ...
              if (data.attack_path_type)
                lastAttackPathType = data.attack_path_type;
              ...
              const parts = [
                `**Phase Transition Request:** ${data.from_phase || '?'} → ${data.to_phase || '?'}`,
              ];
          ...
          if (item._toolConfResponse) {
            ...
            for (let j = i - 1; j >= 0; j--) {
              ...
              if (
                ...
                (prev.type === 'tool_execution' || prev.type === 'plan_wave')
              ) {
      ...
      for (const msg of full.messages) {
        if (msg.type === 'tool_start' && (msg.data as any)?.wave_id) {
          ...
          waveToolStartTimes.set(
            `${d.wave_id}:${d.tool_name}:${si}`,
      ...
      for (const msg of waveToolCompletes) {
        ...
        const waveIdx = restored.findIndex(
          (item) =>
            item.type === 'plan_wave' &&
            (item as PlanWaveItem).wave_id === data.wave_id,
        );
        if (waveIdx !== -1) {
          const wave = restored[waveIdx] as PlanWaveItem;
          const rawOutput = data.raw_output || '';
          const si = data.step_index ?? '';
          const startTime = waveToolStartTimes.get(
            `${data.wave_id}:${data.tool_name}:${si}`,
          );
          const completeTime = new Date(msg.createdAt);
          const duration = startTime
            ? completeTime.getTime() - startTime.getTime()
          ...
          const newTools = [
            ...wave.tools,
            {
              ...
              tool_name: data.tool_name || '',
              tool_args: data.tool_args || {},
              status: (data.success ? 'success' : 'error') as
                | 'success'
                | 'error',
              output_chunks: rawOutput ? [rawOutput] : [],
              final_output: data.output_summary,
      ...
      const planCompletes = full.messages.filter(
        (m: any) => m.type === 'plan_complete',
      );
      ...
      const planAnalyses = full.messages.filter(
        (m: any) => m.type === 'plan_analysis',
      );
      ...
      try {
        const ftRes = await fetch(
          `/api/conversations/by-session/${conv.sessionId}/fireteams`,
        );
        if (ftRes.ok) {
        ...
        for (const msg of full.messages as any[]) {
          ...
          switch (msg.type) {
            ...
            case 'fireteam_plan_complete': {
              mutate(ftId, mId, (m) => {
                const idx = m.planWaves.findIndex(
                  (w: any) => w.wave_id === d.wave_id,
                );
                if (idx < 0) return m;
              ...
              mutate(ftId, mId, (m) => {
                if (d.wave_id) {
                  ...
                  if (idx >= 0) {
                    ...
                    nextWaves[idx] = {
                      ...nextWaves[idx],
                      tools: [...nextWaves[idx].tools, newTool],
                    };
                    return { ...m, planWaves: nextWaves };
                  }
                }
            ...
            case 'fireteam_tool_complete': {
              mutate(ftId, mId, (m) => {
                ...
                // Nested in plan wave?
                if (d.wave_id) {
                  const wi = m.planWaves.findIndex(
                    (w: any) => w.wave_id === d.wave_id,
                  );
                  if (wi >= 0) {
                    ...
                    for (let ti = wave.tools.length - 1; ti >= 0; ti--) {
                      if (
                        wave.tools[ti].tool_name === d.tool_name &&
                        wave.tools[ti].status === 'running'
                      ) {
                ...
                for (let ti = m.tools.length - 1; ti >= 0; ti--) {
                  if (
                    m.tools[ti].tool_name === d.tool_name &&
                    m.tools[ti].status === 'running'
                  ) {
            ...
            t.status === 'running' || t.status === 'pending_approval'
              ? {
                  ...
                  duration:
                    ...
                    Math.max(
                      0,
                      Date.now() - (t.timestamp?.getTime?.() ?? Date.now()),
                    ),
                }
              : t;
          const nextTools = (m.tools || []).map(flipTool);
          const nextWaves = (m.planWaves || []).map((w: any) => {
            if (w.status !== 'running' && w.status !== 'pending_approval')
              return w;
            return {
              ...w,
      ...
      if (conv.activeSkillId) {
        try {
          const res = await fetch(
            `/api/users/${userId}/chat-skills/${conv.activeSkillId}`,
          );
          if (res.ok) {
      ...
      if (lastToolConfirmationRequest && !hasWorkAfterToolConfirmation) {
        ...
        const pendingTool = restored.findLast?.(
          (item: any) =>
            item.type === 'tool_execution' &&
            item.status === 'pending_approval',
        );
        const pendingWave = restored.findLast?.(
          (item: any) =>
            item.type === 'plan_wave' && item.status === 'pending_approval',
        );
        if (pendingWave) {
