...
export function useSendHandlers(deps: SendHandlersDeps) {
  ...
  const activateSkill = useCallback(
    async (
      ...
      try {
        ...
        addSystemMessage(
          `Failed to load Chat Skill${skillName ? ` '${skillName}'` : ''}. Please try again.`,
        );
        return null;
      }
      ...
      addSystemMessage(
        `[Chat Skill Active: ${fullSkill.name}] Category: ${fullSkill.category}`,
      );
      return fullSkill;
    },
    [
  ...
  const handleSkillCommand = useCallback(
    async (args: string) => {
      ...
      if (!query || query.toLowerCase() === 'list') {
        if (skills.length === 0) {
          addSystemMessage(
            'No Chat Skills found. Create skills in Settings > Chat Skills.',
          );
          return;
        }
        ...
        for (const [cat, items] of Object.entries(grouped).sort(([a], [b]) =>
          a.localeCompare(b),
        )) {
          text += `\n${cat}:\n`;
      ...
      const sortedByNameLength = [...skills].sort(
        (a, b) => b.name.length - a.name.length,
      );
      for (const s of sortedByNameLength) {
        const lowerName = s.name.toLowerCase();
        if (
          lowerQuery === lowerName ||
          lowerQuery.startsWith(lowerName + ' ')
        ) {
          bestMatch = s;
      ...
      if (!bestMatch) {
        ...
        const matches = skills.filter(
          (s) =>
            ...
            s.id.toLowerCase().includes(firstWord),
        );
        ...
        if (matches.length > 1) {
          const list = matches
            ...
            .join('\n');
          addSystemMessage(
            `Multiple Chat Skills match '${firstWord}'. Be more specific:\n${list}`,
          );
          return;
        }
      ...
      if (remainingMessage) {
        ...
        if (isLoading) {
          ...
          const hasUserMessage = chatItems.some(
            (item: ChatItem) => 'role' in item && item.role === 'user',
          );
          if (!hasUserMessage) {
  ...
  const handleApproval = useCallback(
    (decision: 'approve' | 'modify' | 'abort') => {
      ...
      try {
        sendApproval(
          decision,
          decision === 'modify' ? modificationText : undefined,
        );
        setModificationText('');
      } catch {
        setIsLoading(false);
  ...
  const handleTimelineToolConfirmation = useCallback(
    (itemId: string, decision: 'approve' | 'reject') => {
      ...
      if (fireteamHit) {
        ...
        try {
          sendFireteamMemberConfirmation(
            fireteamHit.fireteamId,
            fireteamHit.memberId,
            decision,
          );
        } catch {
  ...
  const handleToolStop = useCallback(
    (itemId: string) => {
      let toolName: string | null = null;
      let waveId: string | undefined;
      let stepIndex: number | undefined;

      outer: for (const it of chatItems) {
        if (!('type' in it)) continue;
        ...
        prev.map((item: ChatItem) => {
          ...
          if (item.type === 'fireteam') {
            ...
            const nextMembers = ft.members.map((member) => {
              ...
              const pwIdx = member.planWaves.findIndex((pw) =>
                pw.tools.some((t) => t.id === itemId),
              );
              if (pwIdx !== -1) {
