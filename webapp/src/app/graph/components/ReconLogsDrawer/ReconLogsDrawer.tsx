  ...
  const getStatusText = () => {
    switch (status) {
      case 'starting':
        return '시작 중...';
      case 'running':
        if (!currentPhase) return '실행 중...';
        return hidePhaseProgress
          ? `스칄닝 중: ${currentPhase}`
          : `페이즈 ${currentPhaseNumber}/${totalPhases}: ${currentPhase}`;
      case 'paused':
        if (!currentPhase) return '일시 중지';
        return hidePhaseProgress
          ? `일시 중지: ${currentPhase}`
          : `일시 중지 — 페이즈 ${currentPhaseNumber}/${totalPhases}: ${currentPhase}`;
      case 'completed':
        return '완료';
      case 'error':
        return errorMessage ? `오류: ${errorMessage}` : '오류';
      case 'stopping':
        return '중지 중...';
      default:
        return '대기 중';
        ...
        <div className={styles.statusActions}>
          {(status === 'running' || status === 'paused') && (
            <button
              ...
              title={
                status === 'paused' ? '파이프라인 재개' : '파이프라인 일시 중지'
              }
            >
      ...
      {!hidePhaseProgress && (
        <div className={styles.phaseProgress}>
          {phases.map((phase, index) => {
            ...
            const isCompleted =
              currentPhaseNumber !== null && phaseNum < currentPhaseNumber;
            const isPending =
              currentPhaseNumber === null || phaseNum > currentPhaseNumber;
