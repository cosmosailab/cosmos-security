...
export function TopVulnTechBubble({ data, isLoading }: TopVulnTechBubbleProps) {
  ...
  const chartData = useMemo(() => {
    ...
    const byTech = new Map<
      string,
      { cves: Set<string>; maxCvss: number; criticals: number }
    >();
    for (const chain of data) {
      const existing = byTech.get(chain.tech);
      if (existing) {
        existing.cves.add(chain.cveId);
        if (chain.cvss != null && chain.cvss > existing.maxCvss)
          existing.maxCvss = chain.cvss;
        if (chain.cveSeverity?.toLowerCase() === 'critical')
          existing.criticals++;
      } else {
        ...
        <ScatterChart margin={{ left: 0, right: 8, top: 8, bottom: 8 }}>
            ...
            content={({ payload }) => {
              ...
              return (
                <div style={{ ...tooltipStyle, padding: '8px 12px' }}>
                  ...
                  <div style={{ fontSize: 11 }}>최대 CVSS: {d.maxCvss}</div>
                  <div style={{ fontSize: 11 }}>치명적: {d.criticalCount}</div>
                </div>
              );
            }}
          />
          <Scatter data={chartData} isAnimationActive={false}>
            {chartData.map((entry, i) => (
              <Cell key={i} fill={palette[i % palette.length]} opacity={0.75} />
            ))}
          </Scatter>
        </ScatterChart>
      </ResponsiveContainer>
