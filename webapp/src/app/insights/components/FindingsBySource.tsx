...
export function FindingsBySource({ data, isLoading }: FindingsBySourceProps) {
  ...
  const chartData = useMemo(() => {
    ...
    return [...map.values()].sort((a, b) => {
      const totalA = SEVERITIES.reduce(
        (s, k) => s + ((a as Record<string, number>)[k] || 0),
        0,
      );
      const totalB = SEVERITIES.reduce(
        (s, k) => s + ((b as Record<string, number>)[k] || 0),
        0,
      );
