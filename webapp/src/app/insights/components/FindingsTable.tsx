...
export function FindingsTable({ data, isLoading }: FindingsTableProps) {
  ...
  const getTargetDisplay = (f: SecurityFinding) => {
    if (f.endpointPath)
      return f.endpointPath + (f.paramName ? `?${f.paramName}` : '');
    if (f.target) return f.target;
    if (f.host) return f.host;
    if (f.targetIp)
      return f.targetIp + (f.targetPort ? `:${f.targetPort}` : '');
    return '-';
  };
      ...
      <div className={styles.tableWrap}>
        <table className="table">
          <thead className="tableHeader">
            <tr>
              <th
                className="tableHeaderCell tableHeaderSortable"
          ...
          <tbody>
            {sorted.map((row, i) => (
              <tr key={i} className="tableRow">
                ...
                <td className="tableCell tableCellTruncate" title={row.name}>
                  {row.name}
                </td>
                <td
                  className="tableCell tableCellMono tableCellTruncate"
                  title={getTargetDisplay(row)}
                >
                  {(() => {
