...
export function PortServiceHeatmap({
  services,
  isLoading,
}: PortServiceHeatmapProps) {
  useTheme();
  ...
  const { uniqueServices, uniquePorts, matrix, maxCount } = useMemo(() => {
    ...
    const sortedPorts = Array.from(portSet)
      .sort((a, b) => a - b)
      .slice(0, 15);

    return {
      uniqueServices: sortedSvcs.slice(0, 12),
      uniquePorts: sortedPorts,
      matrix: mat,
      maxCount: max,
    };
  }, [services]);
  ...
  return (
    <ChartCard
      title="포트 × 서비스 매트릭스"
      subtitle={`${uniqueServices.length}개 서비스 × ${uniquePorts.length}개 포트`}
      isLoading={isLoading}
      isEmpty={isEmpty}
    >
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.headerRow}>
            {uniquePorts.map((port) => (
              <div key={port} className={styles.headerCell}>
                {port}
              </div>
            ))}
          </div>
          ...
          {uniqueServices.map((svc) => (
            <div key={svc} className={styles.row}>
              <div className={styles.rowLabel} title={svc}>
                {svc}
              </div>
              {uniquePorts.map((port) => {
