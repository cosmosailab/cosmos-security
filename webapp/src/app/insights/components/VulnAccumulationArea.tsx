...
interface VulnAccumulationAreaProps {
  data:
    | { date: string; count: number; [severity: string]: unknown }[]
    | undefined;
  isLoading: boolean;
}
...
export function VulnAccumulationArea({
  data,
  isLoading,
}: VulnAccumulationAreaProps) {
          ...
          <defs>
            {activeSeverities.map((sev) => (
              ...
              >
                <stop
                  offset="5%"
                  stopColor={sevPalette[sev as keyof typeof sevPalette]}
                  stopOpacity={0.4}
                />
                <stop
                  offset="95%"
