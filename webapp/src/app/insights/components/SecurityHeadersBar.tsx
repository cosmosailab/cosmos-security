...
export function SecurityHeadersBar({
  data,
  isLoading,
}: SecurityHeadersBarProps) {
  ...
  const secCount = useMemo(
    () => data?.filter((d) => d.isSecurity).length || 0,
    [data],
  );
  ...
  return (
    <ChartCard
      title="보안 헤더"
      subtitle={`보안 ${secCount}개 / 기타 ${(data?.length || 0) - secCount}개 (전체 ${data?.length || 0}개)`}
      isLoading={isLoading}
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(
              value: number,
              _name: string,
              props: { payload?: { isSecurity: boolean } },
