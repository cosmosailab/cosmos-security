...
export function SecurityPostureRadar({
  ...
  const chartData = useMemo(() => {
    ...
    const totalVulns =
      vulnData?.severityDistribution?.reduce((s, d) => s + d.count, 0) || 0;
    const totalCves =
      vulnData?.cveSeverity?.reduce((s, d) => s + d.count, 0) || 0;
    const secretsCount =
      (vulnData?.githubSecrets?.secrets || 0) +
      (vulnData?.githubSecrets?.sensitiveFiles || 0);
    ...
    const exploitCount =
      gvmExploits + exploitSuccessCount + kevCount + cvesWithCapec;
    ...
    const attackSurfaceRaw =
      graphData.subdomainStats.total +
      infraStats.totalIps +
      openPorts +
      graphData.endpointCoverage.baseUrls +
      graphData.endpointCoverage.endpoints +
      graphData.endpointCoverage.parameters +
      techCount;
    // Injectable ratio: injectable params / total params across all positions
    const totalParams =
      surfaceData?.parameterAnalysis?.reduce((s, p) => s + p.total, 0) || 0;
    const injectableParams =
      surfaceData?.parameterAnalysis?.reduce((s, p) => s + p.injectable, 0) ||
      0;
    ...
    const certHealthy = Math.max(
      0,
      certTotal -
        graphData.certificateHealth.expired -
        graphData.certificateHealth.expiringSoon,
    );
    ...
    return [
      ...
      },
      {
        metric: 'Vuln Density',
        value: logNorm(
          totalVulns + totalCves + secretsCount + chainFindingsCount,
          15,
        ),
      ...
      },
      {
        ...
        value:
          totalParams > 0
            ? Math.round((injectableParams / totalParams) * 100)
            : 0,
