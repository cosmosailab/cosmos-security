...
export function RiskScoreGauge({
  ...
  const { score, color, label } = useMemo(() => {
    ...
    const vulnScore = (vulnData.severityDistribution || []).reduce(
      (sum, d) => sum + d.count * severityWeight(d.severity),
      0,
    );
    ...
    const kevScore =
      (vulnData.exploits?.filter((e) => e.cisaKev)?.length || 0) * 120;
    ...
    const sensitiveFilesScore =
      (vulnData.githubSecrets?.sensitiveFiles || 0) * 30;
    const injectableCount =
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          12 signals: vulns, CVEs, exploits, KEV, chains, secrets, injection
          &amp; headers
        </div>
      </div>
    </ChartCard>
